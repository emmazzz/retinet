...
def conv_layer_forward(input, layer, param):
  ...
  assert h_out == int(np.floor(h_out)), 'h_out is not integer (error occured at convolution forward)'
  assert w_out == int(np.floor(w_out)), 'w_out is not integer (error occured at convolution forward)'
...
def pooling_layer_forward(input, layer):
  ...
  for n in range(batch_size):
    ...
    index = 0
    temp_out = np.zeros((h_out*w_out,c))
    for row in col.transpose():
        
        tmp = row.reshape((k*k,c))
        tmp_max = [max(x) for x in tmp.transpose()]
        temp_out[index] = tmp_max
        index+=1

    output['data'][:,n] = temp_out.flatten()
...
def relu_forward(input, layer):
  ...
  # implementation begins
  rectifier = lambda t: t if t > 0 else 0
  vfunc = np.vectorize(rectifier)
  output['data'] = vfunc(input['data'])
...
def inner_product_forward(input, layer, param):
  ...
  # implementation begins
  output['data'] = np.dot(param['w'].transpose(), (input['data'])) + np.reshape(param['b'], (num, 1))
...
def mlrloss_predict(wb, X, K, prediction):
  """Loss layer
